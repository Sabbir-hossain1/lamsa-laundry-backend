...
class _ProductDropdownSerializer(serializers.ModelSerializer):
    label = serializers.CharField(source="name", read_only=True)
    value = serializers.IntegerField(source="id", read_only=True)
...
class AdminPriceDetailSerializer(serializers.ModelSerializer):
    """For retrieving full price details."""

    service = ServiceDropdownSerializer()
    product = _ProductDropdownSerializer()
